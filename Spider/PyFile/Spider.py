...
def exchange_rate_detect():
    ...
    exchange_regex=r'div class="price"\D+\d+.\d+\D+-- no spaces'
...
def weather_detect():
    ...
    html=str(connection.read())
    weather_pattern=r'Houston, TX\D+"wslms-degree">\d+\D+"wslms-outlook">\D+</span></a></li><li'
...
def export():
    file=open('.\\Spider_Log.csv', 'at')
    record=str(t)+','+str(brent)+','+str(wti)+','+str(exchange)+','+str(bitcoin)+','+str(degree)+','+str(outlook)+','+str(gold)+','+str(pm)+','+str(level)+'\n'
    file.write(record)
    file.close()
...
if __name__=='__main__':    
    ...
    label_perbarrel0.place(x=280,y=350)

    #PerBarrel Label    
    label_perbarrel1=tkinter.Label(root,text='/bbl.', font=30)
    label_perbarrel1.place(x=580,y=350)
    ...
    label_perbitcoin.place(x=580,y=450)
